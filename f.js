function ETFreeGetStyle(c, a) {
    return c.currentStyle ? c.currentStyle[a] : document.defaultView && document.defaultView.getComputedStyle ? document.defaultView.getComputedStyle(c, "")[a] : c.style[a]
}
function ETFreetrackercode() {
    var c = 0;
    "undefined" !== typeof stopeXTReMeTracking && 1 === stopeXTReMeTracking && (c = 1);
    if (!c) {
        c = document;
        for (var a = c.getElementsByTagName("script"), f, e, g, b = 0; b < a.length; b++)
            a[b].src && -1 < a[b].src.indexOf("t1.extreme-dm.com/f.js") && (e = a[b].id.split("-"), g = a[b].id, f = e[1], e = e[2]);
        if (f && (a = "", "0" === e ? a = 9 : "1" === e ? a = 10 : "2" === e && (a = 11), a)) {
            var d = document.createElement("a");
            d.href = "//extremetracking.com/free?login=" + f;
            d.target = "_blank";
            d.id = "eXTReMe-Free-" + f;
            if (!document.getElementById(d.id)) {
                d.title =
                "eXTReMe Tracker - Free Website Statistics";
                d.innerHTML = '<img src="data:image/gif;base64," style="border:0;border-radius:4px;height:38px;width:41px;" alt="' +
                d.title + '">';
                b = c.getElementById(g).parentNode;
                "HEAD" === b.tagName ? window.addEventListener && window.addEventListener("load", function(a) {
                    document.body.appendChild(d)
                }, !1) : b.insertBefore(d, c.getElementById(g).nextSibling);
                b.style.visibility = "visible";
                "none" === ETFreeGetStyle(b, "display") && (b.style.display = "block");
                g = c.referrer;
                try {
                    g = top.document.referrer
                } catch (h) {}
                f = "login=" + f + "&srw=" + screen.width + "&srh=" + screen.height + "&jv=" + navigator.javaEnabled() + "&j=y&srb=" + screen.colorDepth + "&l=" + escape(g);
                b = new Image;
                b.src = "//e" + e + ".extreme-dm.com/s" + a + ".g?" + f
            }
        }
    }
}
ETFreetrackercode();

